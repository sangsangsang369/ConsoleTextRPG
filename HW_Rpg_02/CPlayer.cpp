...
{
	m_pInventory = CInventory::Create(_pInventory, _iMoney);
}
...
void CPlayer::Render() const
{
	...
	cout << "캐시 : " << m_pInventory->Get_m_iMoney() << endl;
}
...
void CPlayer::RenderInventory() const
{
	...
	for (int i = 0; i < ALL_ITEM_TYPE_NUM; i++)
	{
		...
		cout << "소지 갯수 : " << m_pInventory->Get_m_tInventoryItem(i).iNumInInventory << endl;
	}
}
